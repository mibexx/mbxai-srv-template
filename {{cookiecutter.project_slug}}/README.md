...
- They allow you to point `OAuth2Configuration.authorization_endpoint`, `token_endpoint`, and a dynamic client registration endpoint at the service itself for quick experiments.
- In real projects, you can **override or extend** the behavior by editing `src/{{cookiecutter.package_name}}/mcp/auth.py` (for example to delegate to an external IdP, perform real login/consent, issue signed JWTs, or persist client registrations), while the rest of the template (MCP server wiring and tool security metadata) stays unchanged.

#### Customizing the OAuth2 flow for your project
...
4. **Customize dynamic client registration in `/oauth/register`**
   ...
   - Extend `register_client` in `mcp/auth.py` to:
     ...
     - Persist the client (for example in a database) together with its secrets and metadata.
     - Optionally implement `GET/PUT/DELETE` on `registration_client_uri` for managing existing clients, reusing the `registration_access_token` for authorization as described in RFC 7591.

5. **Wire scopes and security into MCP tools**
   - Use `MCP_OAUTH2_SCOPES_SUPPORTED` to declare which scopes your MCP server understands.
   - When registering tools in `mcp/app.py`, pass `security_schemes` with the scopes that each tool requires.
   - Inside your tools, inspect the injected `__auth__` context (for example `access_token`, `scopes`, user identifiers) to enforce any additional project-specific authorization logic.
